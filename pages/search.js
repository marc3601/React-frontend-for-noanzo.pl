    ...
    fetch("https://admin.noanzo.pl/api/auctions")
      ...
      .then((data) => {
        data.forEach((element) => {
          const lowerCaseItem =
            element.title !== undefined && element?.title.toLowerCase();
          const userQuery = query.toLowerCase();
          const isPresent =
            typeof lowerCaseItem === "string"
              ? lowerCaseItem?.includes(userQuery)
              : false;
          if (isPresent) {
  ...
  return (
    <>
      <Head>
        ...
        <title>
          {query.length > 0 ? query + " - noanzo.pl" : "Nic nie znaleziono"}{" "}
        </title>
      </Head>
      <Layout>
        <Navigation />
